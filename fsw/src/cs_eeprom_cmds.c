...
void CS_GetEntryIDEepromCmd(const CS_GetEntryIDCmd_t *CmdPtr)
{
    ...
    for (i = 0; i < CS_MAX_NUM_EEPROM_TABLE_ENTRIES; i++)
    {
        if ((CS_AppData.ResEepromTblPtr[i].StartAddress <= CmdPtr->Payload.Address) &&
            CmdPtr->Payload.Address <=
                (CS_AppData.ResEepromTblPtr[i].StartAddress + CS_AppData.ResEepromTblPtr[i].NumBytesToChecksum) &&
            CS_AppData.ResEepromTblPtr[i].State != CS_STATE_EMPTY)
        {
            CFE_EVS_SendEvent(CS_GET_ENTRY_ID_EEPROM_INF_EID, CFE_EVS_EventType_INFORMATION,
                              "EEPROM Found Address 0x%08X in Entry ID %d", (unsigned int)(CmdPtr->Payload.Address), i);
            EntryFound = true;
        }
    }
