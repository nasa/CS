...
for file_index = 1 to CFE_ES_MAX_APPLICATIONS do
  if (SCX_CPU1_ES_ALE[file_index].ES_AL_AppName = CSAppName) then
    found_app1 = TRUE
  elseif (SCX_CPU1_ES_ALE[file_index].ES_AL_AppName = "TST_CS") then
    found_app2 = TRUE
  endif
enddo
...
if (found_app2 = FALSE) then
  ...
  write ";*********************************************************************"
  ut_setupevents "SCX", "CPU1", "CFE_ES", CFE_ES_START_INF_EID, "INFO", 1
  ut_setupevents "SCX", "CPU1", "TST_CS", TST_CS_INIT_INF_EID, "INFO", 2
